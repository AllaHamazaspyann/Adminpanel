...
import { editCredentials } from './actions';
import { validator } from '../../../helpers/validator';

export const Profile = (props) => {
  const dispatch = useDispatch();

  const userEmail = useSelector(state=> state.registration);
  const userCreedentials = useSelector(state => state.profile);
  const fetchUser = (arg) => dispatch(editCredentials(arg));
  ...
  return (
    <S.Container>
      ...
      <S.Form
          ...
          initialValues={{
            email: userCreedentials.email, 
            password: userCreedentials.password, 
            nickname: userCreedentials.nickname}}
